...
namespace Tergie.source
{
    ...
    {
        /// <summary>
        /// Create a 2d char arrar from a text file.
        /// </summary>
        ...
        public static void Blit(char[,] source, char[,] dest, Vector2I pos)
        {
            int startX = pos.X;
            int startY = pos.Y;
            int width = source.GetLength(dimension:1);
            int height = source.GetLength(dimensions:0);
            for (int i = 0; i < height; i++)
            {
        ...
        {
            for (int i = 0; i < charArray.GetLength(dimension:0); i++)
            {
                for (int j = 0; j < charArray.GetLength(dimension:1); j++)
                {
                    charArray[i, j] = value
                }
            }
